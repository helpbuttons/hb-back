...
export class NetworkRepository extends DefaultCrudRepository<
  ...
  NetworkRelations
> {
  public readonly buttons: HasManyThroughRepositoryFactory<
    Button,
  ...
  public readonly templateButtons: HasManyThroughRepositoryFactory<
    TemplateButton,
  ...
  constructor(
    ...
    @repository.getter('ButtonsNetworkRepository')
    protected buttonsNetworkRepositoryGetter: Getter<ButtonsNetworkRepository>,

    @repository.getter('ButtonRepository')
    protected buttonRepositoryGetter: Getter<ButtonRepository>,

    @repository.getter('TemplateButtonNetworkRepository')
    protected templateButtonNetworkRepositoryGetter: Getter<TemplateButtonNetworkRepository>,

    @repository.getter('TemplateButtonRepository')
    protected templateButtonRepositoryGetter: Getter<TemplateButtonRepository>,
  ) {
    super(Network, dataSource);

    this.templateButtons = this.createHasManyThroughRepositoryFactoryFor(
      'templateButtons',
    ...
    this.registerInclusionResolver(
      'templateButtons',
      this.templateButtons.inclusionResolver,
    );

    this.buttons = this.createHasManyThroughRepositoryFactoryFor(
      'buttons',
      buttonRepositoryGetter,
      buttonsNetworkRepositoryGetter,
    );
  ...
  public findOwnerNetworks(userId: string, networkIds: number[]) {
    return this.find({
      where: {id: {inq: networkIds}, owner: userId},
      fields: {id: true},
    }).then(networks => {
  ...
  public isOwner(userId: string, networkId: number) {
    return this.find({
      where: {id: networkId, owner: userId},
      fields: {id: true},
    }).then(networks => {
