...
describe('ButtonController (integration)', () => {
  ...
  describe('/buttons/find', () => {
    ...
    before('find buttons', async () => {
      ...
      await client
        ...
        .set('Authorization', 'Bearer ' + token)
        .query({networks: JSON.stringify(networks)})
        .expect(200);
    });
    ...
    it('/buttons/find with networks', async () => {
      const resFilterOne = await client
        .get('/buttons/find')
        .query({
          filter: '{"where": {"id":' + buttonId + '}, "include":["networks"]}',
        })
        .set('Authorization', 'Bearer ' + token)
        .expect(200);
    ...
    it('/buttons/find without networks', async () => {
      const resFilterOne = await client
        .get('/buttons/find')
        .query({filter: '{"where": {"id":' + buttonId + '}}'})
        .set('Authorization', 'Bearer ' + token)
        .expect(200);
  ...
  describe('/buttons/delete', () => {
    ...
    it('/buttons/delete/{id}', async () => {
      ...
      await client
        .delete('/buttons/delete/' + buttonId)
        .set('Authorization', 'Bearer ' + token)
        .expect(204);
  ...
  describe('/buttons/addToNetworks/{buttonId}', () => {
    ...
    it('/buttons/addToNetworks', async () => {
      ...
      await client
        .post('/buttons/addToNetworks/' + buttonId)
        .set('Authorization', 'Bearer ' + token)
        .query({networks: JSON.stringify(networtsToAddIds)})
        .expect(200);
  ...
  describe('transfer ownership of button', () => {
    ...
    it('/buttons/edit/{buttonId}', async () => {
      ...
      await client
        ...
        .send({name: 'yet another new name'})
        .expect(204);
